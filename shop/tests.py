...
class TestDataBase(TestCase):
  ...
  def test_cart_older_7_days(self):
    ...
    cart.creation_time = timezone.datetime(1970, 1, 1, tzinfo=zoneinfo.ZoneInfo('UTC'))
    cart.save()
    cart = Order.get_cart(self.user)
  ...
  def test_after_payment_after_apply_make_order_true(self):
    """check aoto payment after applying make_order()
    1. Tere is a required amount
    """
    Order.objects.all().delete()
    cart = Order.get_cart(self.user)
    OrderItem.objects.create(order=cart, product=self.p, price=2, quantity=2)
  ...
  def test_after_payment_after_apply_make_order_false(self):
    """check aoto payment after applying make_order()
    1. Tere isn't a required amount
    """
    Order.objects.all().delete()
    cart = Order.get_cart(self.user)
  ...
  def test_aoto_payment_after_add_required_payment(self):
  ...
  def test_aoto_payment_for_earlier_order(self):
    """There is unpaid order==556 and balance=0
